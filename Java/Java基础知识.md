...
> **前提**
>
> - **我们异常处理一般只对`编译时异常`进行处理,而对于`Error`以及`运行时异常`一般都不通过异常处理机制来进行解决.**
>
...
> **应用场景**
>
> - 数据库连接,文件输入输出流等是我们的`Java`无法自动回收关闭的,因此我们可以借助`finally`保证我们的程序**无论如何都会执行这些连接的关闭操作**
>
> **经验**
>
> - 对于**运行时异常**,由于其**不确定性**,我们无法确定使用何种方式来对异常处理(**不同的代码出错可能会引发同一种错误**),因此**一般不对其**进行`try catch finally`的**异常处理**.
> - 我们**一般只对编译时异常**进行`try catch finally`异常处理,从而**使得我们的程序能够正确编译**
> - **同理于`throws`关键字的使用**
...
> - :two:由于`try catch finally`是使用了`{}`的,因此在`{}`中的代码(无论是``try,catch,finally``中的哪一个)相对于我们的方法而言是代码块,因此与我们的`C/C++`相同,在这个代码块中创建的**所有**`变量,类实例,数组等结构`**都会**在该代码块**运行完毕后被我们的`Java`垃圾回收程序`自动回收`**.**因此我们在代码块中定义的结构出了代码块之后就`无法被调用了`**
>   - ==**注意**:如果是在我们的**方法层级下定义的结构**,代码块**对结构进行了修改**,那么就有如下情况==
>     - 对于**没有发生异常的修改操作**,那么其**会正确地**作用到我们指定的结构上
>     - 对于**发生了异常的修改操作**,那么其就**不会作用**到我们指定的结构上(类似于**`MySQL`的回滚**)
> - :three:`finally`中的代码是**无论如何一定会被执行的**
>   - **即便**我们的`try`代码块中或`catch`代码块中**使用了`return`**或者**`catch`代码块中存在错误**导致程序报错`finally`都会执行
> - :four:`try catch finally`的结构是**支持嵌套**的,即我们可以在`try`,`catch`,`finally`的代码块中继续添加`try catch finally`结构
...
- :one:执行**`try`代码块**中的语句**,如果报错**就进入:two:,如果**没有报错**则直接跳过`catch`执行后续的代码
- :two:根据报错的异常**依次匹配**`catch`,找到第一个匹配上的`catch`并进入:three:
- :three:执行**匹配上的`catch`的代码块**
  - 当然如果**没有匹配上**则直接**终止我们程序的运行**

#### 有`finally`时

- :one:执行**`try`代码块**中的语句,如果报错就进入:two:,如果**没有报错**则直接跳过`catch`执行`try catch finally`结构后续的代码
- :two:根据报错的异常依次匹配`catch`,找到第一个匹配上的`catch`
- :three:如果**匹配上了**`catch`那么执行匹配上的`catch`的代码块并进入:four:,如果**没有匹配上**则在执行完:four:后**直接中断程序的执行**
- :four:执行`finally`代码块中的代码
- :five:最后继续执行我们的`try catch finally`结构后续的代码
...
> - **向下转型是`不会自动进行`的,必须我们`手动执行`**,并且`强制向下转型`也**只不过**是将**原本父类的指向子类实例**的多态变量指针转变为子类指针.**假若是父类指针指向父类实例**,那么我们是**无法**借助强制向下转型将该父类指针转变为子类指针的.
>
> **问题**
>
...
### 编译时的`try catch finally`的作用

> - 我们知道对于编译时的`Exception`,如果我们不解决就会导致我们编译无法顺利进行
> - 我们可以通过`try catch finally`来捕捉我们的这一`Exception`从而借助其捕获错误并使得程序可以继续运行的特性,使得我们的程序可以正确编译通过
> - **注意**:当然如果我们**对`Exception`的处理不得当**,则会导致我们只是将编译时就会发生的异常转变为在运行时才爆出的异常.
...
> **特性**
>
> - 当遇到错误后,**不进行自动处理**,而是**将我们当前方法遇到的异常抛给我们`调用该方法的方法`**
>
> ==**使用不使用`throws`的区别**==
>
> - 如果不使用`throws`,那么我们当前方法内部出现的`编译时异常`就**必须**在我们的**当前方法内部**使用`try catch finally`结构进行捕获解决,即便是在调用了该方法的方法中捕获解决**也无济于事**
> - 而如果使用`throws`,那么我们当前方法内部出现的`编译时异常`就**既可以**在当前方法内部进行捕获解决,**又可以**在调用了该方法的方法内部进行捕获解决.(**当然如果方法内部没有进行捕获解决,那么调用该方法的方法就必须对该异常进行捕获解决,否则无法编译**)
>
> **基本原理**
>
> - 当我们的当前方法调用了`throws`的方法,若该**被调用方法运行过程中发生了异常**就会进入如下流程
>   - 首先该**被调用的方法**会**生成**一个系统曝出的的**异常类型的类实例**
>   - 然后该异常类实例就会被**抛出给`调用该方法的方法`**,并且我们当前**被调用的**方法**曝出异常的代码之后的代码便`不再执行`**
>
> **注意事项**
>
> - **一般我们只对`编译时异常`使用`throws`**
>   - 原因在于`运行时异常`即便使用了`throws`也和没有使用效果相同

### **`throws`指定与不使用`throws`指定的区别**

> **不同点**
>
> - :one:对于`throws`指定**编译时异常**而言,在`方法C`中**手动**抛出或系统**自动**抛出`编译时异常`的情况下,我们**必须保证**对于`方法A调用方法B,方法B调用方法C`且`方法B,C`都使用了`throws`对该异常进行了指定的这样一种情况下有
>   - 我们必须保证**要么**`方法C`中存在`try catch finally`结构来捕获这个异常,**要么**`方法B`中存在该结构来捕获这个异常,**要么**`方法A`中存在该结构来捕获这个异常.
>   - 如果`方法A,B`**均不捕获**该异常,那么我们**便一定无法正常编译**
> - :two:同样的情况,若只有`方法C`使用了`throws`的情况下,就有
>   - `方法B`或`方法C`两者中**至少要有一个一定要**使用`try catch finally`来对该异常进行捕获
> - :three:同样的情况,若只有`方法B`使用了`throws`,且异常是`方法B`中抛出的情况下,就有
>   - `方法A`中**一定要**使用`try catch finally`来对该异常进行捕获
> - :four:对于都没有用`throws`指定编译时异常,且没有**手动**抛出或系统**自动**抛出`编译时异常`时,则**没有上述的要求**.我们**可以捕获,也可以不捕获**
> - :five:对于都没有用`throws`指定编译时异常,但在`方法C`中**手动**抛出或系统**自动**抛出`编译时异常`时,我们**必须且只能**在`方法C`中对该异常进行捕获解决

### 基本语法

```java
方法名(形参列表) throws 异常类型1[,异常类型2...]{方法语句体}
```

### 带`throws`的方法的重写规则

- :one:子类重写的方法抛出的异常类型**不可以大于**原始方法的异常类型
  - 原因在于我们的`Java`存在多态性,可以用父类的指针指向子类的实例并调用子类重写后的方法.如果子类重写后的方法的异常类型大于我们的原始方法的异常类型,就会导致`Java`多态使用起来在异常处理方面很不方便.
- :two:当原始方法没有使用`throws`时,我们的子类重写方法就也不能使用`throws`
...
> **概念**
>
> - 在前面我们都是程序在遇到异常时**自动地生成异常对象并抛出**该异常给我们的用户
> - 而手动的异常抛出即我们在代码中的一定位置**借助`throw`关键字手动实例化**异常类型并**抛出给用户**
>
> **注意事项**
>
> - `throw`手动抛出异常与我们的程序遇到异常自动抛出本质上是相同的,因此我们可以通过`try catch finally`以及`throws`来对我们手动抛出的异常进行处理

### 基本语法

> 当`Java`程序**运行到这一语句时**就会生成指定的异常类的类实例,并将该异常抛出给我们的用户
...
> **基本原理**
>
> - 通过创建自己的类来继承`Java`自带内置异常类,并对其进行重写,重定义等操作来实现`用户自定义异常类`

### 基本语法

```java'
public class 自定义异常类名 extends Java内置异常名{}
```
...
> - 对于我们的运行时异常而言,其一般不借助异常处理机制进行处理
> - 对于`方法A调用方法B,方法B调用方法C,方法C调用方法D`的情况,且`方法D`中手动或自动地抛出了一个指定的运行时异常的情况而言有
>   - 若`方法D`中使用了`try catch finally`结构对该异常进行捕获解决,那么就会**由`方法D`解决**,若`方法D`中**没有捕获解决**,那么`方法D`就会被`弹栈`其**后续代码便不会再执**行,然后异常就会被**抛出给我们的`方法C`**
>   - 然后`方法C,方法B`都会经历`方法D`这样的选择
>   - 若`方法B,C,D`**都没有捕获**这个异常,那么异常就会**交付给我们的`方法A`**,若`方法A`捕获并解决了,程序就继续运行,如果`方法A`**没能捕获**解决,那么**`方法A`就会被直接终止,然后该运行时异常就会直接被抛出给我们的用户**
